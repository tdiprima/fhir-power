...
def fetch_fhir_resources(resource_type, query_params=None):
    ...
    try:
        ...
        print(
            f"❌ Error: Request timed out when fetching {resource_type.lower()}. The server took too long to respond."
        )
        return [] if resource_type != "Patient" else sys.exit(1)
    except ConnectionError:
        print(
            f"❌ Error: Connection error when fetching {resource_type.lower()}. Check your network connection."
        )
        return [] if resource_type != "Patient" else sys.exit(1)
...
def main():
    ...
    for entry in patients:
        ...
        observations = fetch_fhir_resources(
